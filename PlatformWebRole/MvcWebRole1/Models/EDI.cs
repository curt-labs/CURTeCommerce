    ...
    public class EDI {
        internal void CreatePurchaseOrder(int id = 0) {
            try {
                ...
                edicontent += "ISA*00*          *00*          *12*" + settings.Get("EDIPhone") + "     *01*809988975      *" + String.Format("{0:yyMMdd}*{0:hhmm}", DateTime.Now) + "*U*00401*" + order.ID.ToString("X9") + "*0*P*>~" + Environment.NewLine;
                edicontent += "GS*PO*" + settings.Get("EDIPhone") + "*809988975*" + String.Format("{0:yyyyMMdd}*{0:hhmm}", DateTime.Now) + "*" + order.ID.ToString("X9") + "*X*004010~" + Environment.NewLine;
                // begin PO section
                edicontent += "ST*850*" + order.ID + "~" + Environment.NewLine;
                linecount++;
                edicontent += "BEG*00*DS*" + order.ID + "**" + String.Format("{0:yyyyMMdd}", order.getPayment().created) + "~" + Environment.NewLine;
                linecount++;
                ...
                linecount++;
                edicontent += "N4*" + order.Shipping.city + "*" + order.Shipping.State1.abbr + "*" + order.Shipping.postal_code + "*" + order.Shipping.State1.Country.abbr + "~" + Environment.NewLine;
                ...
                for (int i = 0; i < order.CartItems.Count; i++) {
                    linecount++;
                    edicontent += "PO1*" + (i + 1).ToString("X3") + "*" + order.CartItems[i].quantity + "*EA***BP*" + order.CartItems[i].partID + "*VP*" + order.CartItems[i].partID + "*UP*" + order.CartItems[i].upc + "~" + Environment.NewLine;
                    linecount++;
                ...
                // end PO section
                // begin Tail section
                edicontent += "GE*1*" + order.ID.ToString("X9") + "~" + Environment.NewLine;
                edicontent += "IEA*1*" + order.ID.ToString("X9") + "~";
